...
export function summarizeNtsTransmission(documents = [], expectedCount = documents.length) {
  const successDocuments = documents.filter((document) => (
    Number(document.stateCode) === 304
    && Boolean(String(document.ntsconfirmNum || '').trim())
    && (!document.ntssendErrCode || String(document.ntssendErrCode) === 'SUC001')
  ));
  const failedDocuments = documents.filter((document) => (
    Number(document.stateCode) === 305
    || (document.ntssendErrCode && String(document.ntssendErrCode) !== 'SUC001')
  ));
  const latest = (field) => documents
    .map((document) => String(document[field] || ''))
    .filter(Boolean)
    .sort()
    .at(-1) || '';
  let status = '미발행';
  if (failedDocuments.length > 0) status = successDocuments.length > 0 ? '국세청 부분실패' : '국세청 전송실패';
  else if (expectedCount > 0 && successDocuments.length === expectedCount) status = '국세청 전송완료';
  else if (successDocuments.length > 0) status = '국세청 부분전송';
  else if (documents.length > 0) status = '국세청 전송대기';
  return {
    status,
    expectedCount,
    confirmedCount: successDocuments.length,
    failedCount: failedDocuments.length,
    sentAt: latest('ntssendDT'),
    resultAt: latest('ntsresultDT'),
    errorCodes: [...new Set(failedDocuments.map((document) => String(document.ntssendErrCode || '305')))],
  };
}
