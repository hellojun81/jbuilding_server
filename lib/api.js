...
import { bankLedgerFilters, bankLedgerWhere } from './bankLedger.js';
...
function remoteDocumentMatches(target, document, remoteDocument) {
  ...
  const documentKeyMatches = String(remoteDocument.invoicerMgtKey || '') === expectedDocumentKey;
  const documentValuesMatch = String(remoteDocument.invoiceeCorpNum || '') === String(target.business_number || '')
  ...
  return (documentKeyMatches || documentValuesMatch)
...
router.patch('/bills/:renter/payment', async (req, res, next) => {
  try {
    ...
    const compCode = await sql.getRenterCode(renter);
    const affectedRows = await sql.transaction(async (query) => {
      if (req.body.finish === 'N') {
        await query(
          'DELETE FROM bank_bill_matches WHERE comp_code = ? AND bill_year = ? AND bill_month = ?',
          [compCode, year, month],
        );
      }
      const [result] = await query(
        'UPDATE jbuildingrentbill SET finish = ? WHERE comp_code = ? AND year = ? AND month = ?',
        [req.body.finish, compCode, year, month],
      );
      return result.affectedRows;
    });
    res.json({ affectedRows });
...
router.get('/tax-invoices', async (req, res, next) => {
  try {
    ...
    const invoices = targets.map((row) => {
      ...
      const matchedRemoteDocuments = row.documents
        .map((document) => remoteDocuments.find((remote) => remoteDocumentMatches(row, document, remote)))
        .filter(Boolean);
      const nts = historyConnected
        ? summarizeNtsTransmission(matchedRemoteDocuments, row.documents.length)
        : { status: '확인필요', expectedCount: row.documents.length, confirmedCount: 0, failedCount: 0, sentAt: '', resultAt: '', errorCodes: [] };
      return {
        ...
        issue_status: historyConnected
          ...
          : '발행이력 확인필요',
        nts_status: nts.status,
        nts_expected_count: nts.expectedCount,
        nts_confirmed_count: nts.confirmedCount,
        nts_failed_count: nts.failedCount,
        nts_sent_at: nts.sentAt,
        nts_result_at: nts.resultAt,
        nts_error_codes: nts.errorCodes.join(', '),
      };
    });
    res.json({
      provider,
  ...
  try {
    ...
    const [bills] = await sql.query(
      `SELECT b.comp_code AS renter_code, r.name AS renter_name,
              ...
              m.id AS existing_match_id, m.bank_transaction_id AS existing_transaction_id,
              mt.deposit_amount AS existing_deposit_amount, mt.remark1 AS existing_payer
         ...
         LEFT JOIN bank_transactions mt ON mt.id = m.bank_transaction_id
...
router.get('/bank/ledger', async (req, res, next) => {
  try {
    const filters = bankLedgerFilters(req.query);
    const { whereSql, values } = bankLedgerWhere(filters);
    const [transactions] = await sql.query(
      `SELECT t.id, t.provider_tid, t.bank_code, t.account_number_masked,
              DATE_FORMAT(t.trade_date, '%Y-%m-%d') AS trade_date,
              DATE_FORMAT(t.trade_datetime, '%Y-%m-%d %H:%i:%s') AS trade_datetime,
              t.deposit_amount, t.withdrawal_amount, t.balance,
              t.remark1, t.remark2, t.remark3, t.remark4, t.memo,
              m.id AS match_id, r.name AS matched_renter_name,
              m.bill_year, m.bill_month
         FROM bank_transactions t
         LEFT JOIN bank_bill_matches m ON m.bank_transaction_id = t.id
         LEFT JOIN jbuildingmng r ON r.key_code = m.comp_code
        WHERE ${whereSql}
    ...
    const [summaryRows] = await sql.query(
      `SELECT COUNT(*) AS transaction_count,
              COALESCE(SUM(t.deposit_amount), 0) AS deposit_total,
              COALESCE(SUM(t.withdrawal_amount), 0) AS withdrawal_total
         FROM bank_transactions t
        WHERE ${whereSql}`,
      values,
    );
    const safeTransactions = normalizeQueryRows(transactions);
    const summary = normalizeQueryRows(summaryRows)[0] || {};
    res.json({
      ...
      truncated: Number(summary.transaction_count || 0) > safeTransactions.length,
    });
  } catch (error) { next(error); }
});
...
router.post('/bank/matches', async (req, res, next) => {
  try {
    if (!['MATCH', 'FORCE_MATCH', 'REPLACE_MATCH'].includes(req.body?.confirmation)) {
    ...
    const match = await sql.transaction(async (query) => {
      ...
      const [existingMatches] = await query(
        `SELECT id, bank_transaction_id FROM bank_bill_matches
          WHERE comp_code = ? AND bill_year = ? AND bill_month = ? FOR UPDATE`,
        [renterCode, year, month],
      );
      if (existingMatches.length > 0) {
        if (Number(existingMatches[0].bank_transaction_id) === transactionId) {
          throw Object.assign(new Error('이미 해당 입금과 매칭된 청구서입니다.'), { status: 409 });
        }
        if (req.body.confirmation !== 'REPLACE_MATCH') {
          throw Object.assign(new Error('기존 입금 매칭을 교체한다는 확인이 필요합니다.'), { status: 409 });
        }
        await query('DELETE FROM bank_bill_matches WHERE id = ?', [existingMatches[0].id]);
      }
      const amount = Number(transactions[0].deposit_amount || 0);
      const total = billTotal(bills[0]);
      const forced = amount !== total;
      if (forced && !['FORCE_MATCH', 'REPLACE_MATCH'].includes(req.body.confirmation)) {
      ...
      const [insert] = await query(
        `INSERT INTO bank_bill_matches
         ...
         VALUES (?, ?, ?, ?, ?, ?)`,
        [transactionId, renterCode, year, month, amount,
          existingMatches.length > 0 ? 'replaced_manual' : (forced ? 'forced_manual' : 'manual')],
      );
