  ...
  try {
    ...
    const [bills] = await sql.query(
      `SELECT b.comp_code AS renter_code, r.name AS renter_name,
        ...
        WHERE b.year = ? AND b.month = ? AND b.finish = 'N' AND m.id IS NULL
        ORDER BY r.name`,
      [year, month],
    );
...
router.post('/bank/matches', async (req, res, next) => {
  try {
    ...
    const match = await sql.transaction(async (query) => {
      const [transactions] = await query('SELECT * FROM bank_transactions WHERE id = ? FOR UPDATE', [transactionId]);
